...
3. Match clusters against **existing books of the collection** (another video may show the same shelf again – typically to catch a missed book): the same rule, a partial title or author and title swapped; a match needs shelf context (matching neighbours, a continuing clip, or a photo), and an author-backed / long identical / swapped reading also counts when ≥ 2 other books of the video match books of the same earlier source (that shelf is filmed again, its neighbours may be misread). A lone title match on another shelf stays a second physical copy. Several strong readings of one book that never share a frame join the same existing book. Details: `clusterObservations` in `merge.ts`.
...
* **Book drawer** (all views): spine photo, cover, title/author/original title, year, publisher, topics chips, description (locale), evidence (frame thumbnail at `firstTimeSec` + confidence meter), owner fields: reading status segmented control, 5-star rating, favourite heart, notes, lent to/at; edit / delete; prev/next navigation; deleting a book (confirmation) moves the drawer on to the next book of the list, so wrong entries can be weeded out while browsing (phones: labelled Delete unless three actions have to fit).
...
* Owner can **add a book manually** and **add more videos** from the page (phone app: a camera button in the catalogue's top bar records another shelf straight into this catalogue; the merge skips books already in it).
