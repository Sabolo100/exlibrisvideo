...
6. **Detections**: one row per (spine, frame that shows it) with the exact tilted rectangle in `bbox.rect` and the left→right `orderInFrame`, so the unchanged merge (§4.4) clusters them; neighbouring spines with the same reading (a false boundary) keep only the stronger one. Crops (§4.5) cut `bbox.rect` upright.
